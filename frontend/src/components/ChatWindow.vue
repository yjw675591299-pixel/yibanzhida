    ...
    <footer class="composer">
      ...
      <div class="input-row">
        <el-input
          ...
          resize="none"
          placeholder="输入症状、科室或就医问题…（Enter 发送，Shift+Enter 换行）"
...
const hello = () => {
  sendRequest('你好', { showUserMessage: false })
}
...
const initMemoryId = () => {
  const stored = localStorage.getItem('memory_id') ?? localStorage.getItem('user_uuid')
  if (stored) {
    uuid.value = Number(stored)
    localStorage.setItem('memory_id', String(uuid.value))
    localStorage.removeItem('user_uuid')
    return
  }
  const memoryId = generateMemoryId()
  localStorage.setItem('memory_id', String(memoryId))
  uuid.value = memoryId
}
